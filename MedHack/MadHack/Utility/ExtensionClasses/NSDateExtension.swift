...
extension Date {
    ...
    func getAge(birthday: String?)-> Int?{
        ...
        guard let date = birthday,let birthdayDate = dateFormater.date(from: date)else {
            return nil
        }
    ...
    static func getExpireDate(dateString: String)-> String{
        let date1 = Date().getDateFromString(dateString: dateString, dateFormatter: DateFormats.DATE_FORMAT_YYYY_MM)
        let formattedDateString = date1.getStringFromDate(date: date1, dateFormatter: DateFormats.DATE_FORMAT_MM_SLASH_YY)
        return formattedDateString
    }
    
    static func getValidThruDate(dateString: String)-> String{
        let date1 = Date().getDateFromString(dateString: dateString, dateFormatter: DateFormats.DATE_FORMAT_YYYY_MM_DD)
        let formattedDateString = date1.getStringFromDate(date: date1, dateFormatter: DateFormats.DATE_FORMAT_MM_SLASH_YY)
        return formattedDateString
    }
    
    static func getValidThruDateForGetACard()-> String{
        let date1 = Date().addMonth(n: 12)
        let formattedDateString = date1.getStringFromDate(date: date1, dateFormatter: DateFormats.DATE_FORMAT_MM_SLASH_YY)
        return formattedDateString
    }
}
